...
class VerMaterias extends StatelessWidget {
  ...
  const VerMaterias({Key key, @required this.mensaje}) : super(key: key);
...
class Materias extends StatelessWidget {
  static Route<dynamic> route(String mensaje) {
    return MaterialPageRoute(
      builder: (context) => Materias(mensaje: mensaje),
    );
  }

  final String mensaje;
  const Materias({Key key, @required this.mensaje}) : super(key: key);
    ...
    if (mensaje == record.profesor) {
      ...
      return Padding(
        ...
        child: Container(
          ...
          child: ListTile(
            title: Text('No tienes registro de materias'),
          ),
        ),
      );
    }
